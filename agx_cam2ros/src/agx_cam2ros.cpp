...
camera_initialize(context_t * ctx)
{
    ...
    printf("Camera Number: (%s), ouput format: (%d x %d)  stride: %d, imagesize: %d, frate: %u / %u",
            ctx->cam_devname,
            fmt.fmt.pix.width,
            fmt.fmt.pix.height,
...
static int
jpeg_encode_proc(context_t *ctx, int fd, ros::Publisher xcam)
{   
    ...
    if (!ctx->jpegenc)
        ERROR_RETURN("Could not create Jpeg Encoder");
    
    if (ctx->perf)
    {
        ctx->iterator_num = PERF_LOOP;
        ctx->jpegenc->enableProfiling();
    }

    unsigned long out_buf_size = ctx->cam_w * ctx->cam_h * 3 / 2;
    uint8_t *out_buf = (unsigned char*)malloc(out_buf_size);

    // unsigned char *out_buf = new unsigned char[out_buf_size];
    int ret = ctx->jpegenc->encodeFromFd(fd, JCS_YCbCr, &out_buf, out_buf_size, ctx->quality);     
    
    for(int i = 0; i < out_buf_size; i++){
        vec.push_back(out_buf[i]);
    }
...
start_capture(context_t * ctx)
{
    ...
    ros::NodeHandle nh;
    ros::Publisher xcam = nh.advertise<sensor_msgs::CompressedImage>(std::string("/gmsl_camera") + ctx->cam_devname + std::string("/compressed"), 0);
    ...
    while (poll(fds, 1, 5000) > 0 && !quit)
    {
        if (fds[0].revents & POLLIN) {
            ...
            jpeg_encode_proc(ctx, ctx->render_dmabuf_fd, xcam);
...
main(int argc, char *argv[])
{
    ...
    ros::init(argc, argv, argv[1], ros::init_options::NoSigintHandler);
